...
rgba *mkraw2rgba(const uint8_t *raw, int width, int height, int depth)
{
   rgba *img;
   int img_size;
...
ia *mkraw2ia(const uint8_t *raw, int width, int height, int depth)
{
   ia *img;
   int img_size;
...
ia *mkraw2i(const uint8_t *raw, int width, int height, int depth)
{
   ia *img = NULL;
   int img_size;
...
uint8_t *mkci2raw(const uint8_t *rawci, const uint8_t *palette, int width, int height, int ci_depth)
{
   uint8_t *raw;
   int raw_size;
...
int mkrgba2raw(uint8_t *raw, const rgba *img, int width, int height, int depth)
{
...
int mkia2raw(uint8_t *raw, const ia *img, int width, int height, int depth)
{
   int size = (width * height * depth + 7) / 8;
...
**/
int mkcomp_rgba(const rgba left, const rgba right) {
 ...
 * Otherwise, return -1
**/
int mkget_color_index(const rgba comp, const rgba *pal, int mask_value, int pal_size) {
   int pal_idx;
   ...
   for (; pal_idx < pal_size; pal_idx++) {
      if (mkcomp_rgba(comp, pal[pal_idx]) == 1) return pal_idx;
   }
...
**/
int mkimgpal2rawci(uint8_t *rawci, const rgba *img, const rgba *pal, const uint8_t *wheel_mask, int raw_size, int ci_depth, int img_size, int pal_size) {
   int img_idx;
   int pal_idx;
   int mask_value;
   ...
   for (img_idx = 0; img_idx < img_size; img_idx++) {
      ...
      pal_idx = mkget_color_index(img[img_idx], pal, mask_value, pal_size);
      if (pal_idx != -1) {
         switch (ci_depth) {
            case 8:
...
int mki2raw(uint8_t *raw, const ia *img, int width, int height, int depth)
{
   int size = (width * height * depth + 7) / 8;
...
int mkrgba2png(unsigned char **png_output, int *size_output, const rgba *img, int width, int height)
{
   int ret = 0;
...
int mkia2png(unsigned char **png_output, int *size_output, const ia *img, int width, int height)
{
   int ret = 0;
...
rgba *mkpng2rgba(unsigned char *png_input, int size_input, int *width, int *height)
{
   rgba *img = NULL;
   int w = 0;
   int h = 0;
...
ia *mkpng2ia(unsigned char *png_input, int size_input, int *width, int *height)
{
   ia *img = NULL;
   int w = 0, h = 0;
   int channels = 0;
...
static int mkpal_find_color(const palette_t *pal, uint16_t val)
{
...
static int mkpal_add_color(palette_t *pal, uint16_t val)
{
   int idx;
   idx = mkpal_find_color(pal, val);
   if (idx < 0) {
      if (pal->used == pal->max) {
...
int mkraw2ci(uint8_t *rawci, palette_t *pal, const uint8_t *raw, int raw_len, int ci_depth)
{
...
const char *mk64graphics_get_read_version(void)
{
   return "stb_image 2.19";
}

const char *mk64graphics_get_write_version(void)
{
