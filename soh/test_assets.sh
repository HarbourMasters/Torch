...
FAIL_COUNT=$(jq -n \
    ...
    --slurpfile assets "$ASSET_JSON" \
'$ref[0] as $r | $gen[0] as $g |
reduce $assets[0][] as $asset (0;
    if ($g[$asset] == null) then . + 1
    elif ($r[$asset] == null) then . + 1
    elif ($g[$asset] != $r[$asset]) then . + 1
    else . end
)')
