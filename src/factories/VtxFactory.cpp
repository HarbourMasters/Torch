...
ExportResult VtxHeaderExporter::Export(std::ostream& write, std::shared_ptr<IParsedData> raw, std::string& entryName,
...
ExportResult VtxCodeExporter::Export(std::ostream& write, std::shared_ptr<IParsedData> raw, std::string& entryName,
                                     YAML::Node& node, std::string* replacement) {
    auto vtx = std::static_pointer_cast<VtxData>(raw)->mVtxs;
    const auto symbol = GetSafeNode(node, "symbol", entryName);
    ...
    if (searchTable.has_value()) {
        ...
        for (int i = 0; i < vtx.size(); ++i) {
            ...
            write << fourSpaceTab << "{{{" << NUM(x) << ", " << NUM(y) << ", " << NUM(z) << "}, " << flag << ", {"
                  << NUM(tc1) << ", " << NUM(tc2) << "}, {" << COL(c1) << ", " << COL(c2) << ", " << COL(c3) << ", "
                  << COL(c4) << "}}},";
        }
        write << "\n" << fourSpaceTab << "},\n";
        ...
        for (int i = 0; i < vtx.size(); ++i) {
            ...
            // {{{ x, y, z }, f, { tc1, tc2 }, { c1, c2, c3, c4 }}}
            write << "{{{" << NUM(x) << ", " << NUM(y) << ", " << NUM(z) << "}, " << flag << ", {" << NUM(tc1) << ", "
                  << NUM(tc2) << "}, {" << COL(c1) << ", " << COL(c2) << ", " << COL(c3) << ", " << COL(c4) << "}}},\n";
        }
...
ExportResult VtxBinaryExporter::Export(std::ostream& write, std::shared_ptr<IParsedData> raw, std::string& entryName,
                                       YAML::Node& node, std::string* replacement) {
    ...
    for (auto v : vtx->mVtxs) {
        if (false && Companion::Instance->GetConfig().gbi.useFloats) {
...
void VtxFactoryUI::DrawUI(const ParseResultData& item) {
    ...
    ImGui::TextDisabled("%zu vertices  \xe2\x80\x94  drag to orbit, shift+drag to pan, \xe2\x8c\x98/Ctrl+scroll to zoom",
                        vtx->mVtxs.size());
