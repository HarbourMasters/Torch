    ...
    switch (format.type) {
        ...
        case TextureType::Palette4bpp: {
            ...
            if (Companion::Instance->GetGBIMinorVersion() == GBIMinorVersion::Mk64) {
                auto tlut = GetSafeNode<std::string>(node,"tlut");
                SPDLOG_INFO("TLUT: {}", tlut);
                ...
                if (palettePtr) {
                    ...
                    int bufferSize = texture->mWidth * texture->mHeight * texture->mFormat.depth;
                    for (auto i = 0; i < bufferSize; i++) {
                        palette.data[i] = palettePtr->mBuffer[i];
                    }


                    ci* imgi = mk64_raw2ci_torch(texture->mBuffer.data(), &palette, texture->mWidth, texture->mHeight, format.depth);

                    if(ci2png(&raw, &size, imgi, texture->mWidth, texture->mHeight)) {
                        throw std::runtime_error("Failed to convert mk64 ci8 texture to PNG");
                    }
...
std::optional<std::shared_ptr<IParsedData>> TextureFactory::parse(std::vector<uint8_t>& buffer, YAML::Node& node) {
    ...
    if (fmt.type == TextureType::TLUT) {
        ...
        auto tluts = Companion::Instance->GetTlutTextureMap();
        SPDLOG_INFO("ADDED: {}", symbol);
        tluts[symbol] = textureData;
        return textureData;
    }
