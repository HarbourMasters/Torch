...
ExportResult MK64::ItemCurveHeaderExporter::Export(std::ostream& write, std::shared_ptr<IParsedData> raw,
...
ExportResult MK64::ItemCurveCodeExporter::Export(std::ostream& write, std::shared_ptr<IParsedData> raw,
                                                 std::string& entryName, YAML::Node& node, std::string* replacement) {
...
ExportResult MK64::ItemCurveBinaryExporter::Export(std::ostream& write, std::shared_ptr<IParsedData> raw,
                                                   std::string& entryName, YAML::Node& node, std::string* replacement) {
    throw std::runtime_error("Decomp ItemCurve is only implemented in decomp.\nuk64 and port use a new system for ease "
                             "of modding and bug fixes.");
    return std::nullopt;
}

std::optional<std::shared_ptr<IParsedData>> MK64::ItemCurveFactory::parse(std::vector<uint8_t>& buffer,
                                                                          YAML::Node& node) {
