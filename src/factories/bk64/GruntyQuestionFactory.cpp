...
// One language's question data. Same shape for every language. Unlike quiz options,
// these split their leading control pair out of the string.
GruntyQuestionLang ParseGruntyBlock(LUS::BinaryReader& reader, const std::string& symbol) {
...
void WriteGruntyLangBlock(LUS::BinaryWriter& writer, const std::vector<DialogString>& text,
                          const std::vector<OptionString>& options) {
    WriteDialogStrings(writer, text);

    writer.Write((uint32_t)options.size());
...
ExportResult BK64::GruntyQuestionBinaryExporter::Export(std::ostream& write, std::shared_ptr<IParsedData> raw,
    ...
    const uint32_t langCount = 1 + static_cast<uint32_t>(gruntyQuestion->mExtraLangs.size());
    writer.Write(langCount);

    // English always goes first
    WriteGruntyLangBlock(writer, gruntyQuestion->mText, gruntyQuestion->mOptions);

    // PAL only: French then German
    for (const auto& lang : gruntyQuestion->mExtraLangs) {
        WriteGruntyLangBlock(writer, lang.text, lang.options);
    }
...
std::optional<std::shared_ptr<IParsedData>> GruntyQuestionFactory::parse(std::vector<uint8_t>& buffer,
    ...
    const auto extraOffsets = SeekQuestionBlock(reader, symbol, "GruntyQuestion", kUsHeader, kPalHeader);
    if (!extraOffsets.has_value()) {
        return std::nullopt;
    }

    auto english = ParseGruntyBlock(reader, symbol);
    ...
    for (const uint16_t offset : *extraOffsets) {
        ...
        extraLangs.push_back(ParseGruntyBlock(reader, symbol));
    }

    return std::make_shared<GruntyQuestionData>(std::move(english.text), std::move(english.options),
                                                std::move(extraLangs));
}
