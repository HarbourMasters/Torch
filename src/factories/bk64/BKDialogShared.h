...
// Both question types share a container. US puts the question right after a 5-byte
// header; PAL uses its own 3-byte header followed by three LE u16 language offsets,
// with English first at byte 9. Either way the reader ends up on the English block.
// Returns false (and logs) when neither header matches.
inline bool SeekQuestionBlock(LUS::BinaryReader& reader, const std::string& symbol, const char* kind,
                              const int8_t (&usHeader)[3], const int8_t (&palHeader)[3]) {
    ...
    if (header1 == palHeader[0] && header2 == palHeader[1] && header3 == palHeader[2]) {
        for (int i = 0; i < 6; i++) {
            reader.ReadUByte(); // EN/FR/DE offsets; EN is where we already are
        }
        return true;
    }
